  ...
  tinymce.init({
	...
	<?php
		if(isset($_POST['submit'])){
			...
			$consulta = "INSERT into prueba(slug,tipo,titular,noticia,foto,descMeta,escritor) values ('". $slug. "','" . $tipo . "','" . $titular . "','" . $noticia . "','". $foto . "','". $descMeta . "','". $escritor . "' )";
				...
				<div style="background-image: url('static/originales/fondonoticia.png'); border: 2px solid black;color: black;">
					<div style="width: 95%;margin:auto; text-align:center">
						<form enctype="multipart/form-data" action="redactores.php" method="POST">
							<br>
							Escritor <p><select name="escritor" required>
											<?php 
												$con = mysqli_connect("PMYSQL110.dns-servicio.com", "varo", "&qdGw743", "6795849_tftesp");
												mysqli_set_charset($con,'UTF8');
												$resultado = $con->query('select nombre from redactores order by nombre');
